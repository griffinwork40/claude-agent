...
class GreenhouseClient {
    ...
    async searchJobs(boardToken, filters = {}) {
        try {
            ...
            return jobs
                .map((job) => this.transformJob(job, boardToken, filters))
                .filter((job) => job !== null);
        }
        catch (error) {
     ...
     */
    async searchMultipleBoards(boardTokens, filters = {}) {
        console.log(`🔍 Searching ${boardTokens.length} Greenhouse boards`);
        const searchPromises = boardTokens.map(boardToken => this.searchJobs(boardToken, filters).catch(error => {
            console.error(`❌ Error searching board ${boardToken}:`, error);
            return [];
        }));
     ...
     */
    async getJobDetails(boardToken, jobId) {
        try {
     ...
     */
    async submitApplication(boardToken, jobId, formData) {
        if (!this.apiKey) {
     ...
     */
    transformJob(job, boardToken, filters) {
        try {
     ...
     */
    async searchJobsDefault(filters = {}) {
        const boardTokens = (0, greenhouse_boards_1.getBoardTokens)();
