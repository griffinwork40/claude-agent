// browser-service/src/greenhouse-client.ts
...
export class GreenhouseClient {
  ...
  async searchJobsDefault(filters: GreenhouseSearchFilters = {}): Promise<JobOpportunity[]> {
    const boardTokens = this.getDefaultBoardTokens();
    return this.searchMultipleBoards(boardTokens, filters);
  }

  /**
   * Get default board tokens
   */
  private getDefaultBoardTokens(): string[] {
    const envBoards = process.env.GREENHOUSE_DEFAULT_BOARDS;
    if (envBoards) {
      return envBoards.split(',').map(token => token.trim()).filter(Boolean);
    }
    return [
      'airbnb', 'stripe', 'shopify', 'github', 'gitlab', 'atlassian', 'dropbox',
      'uber', 'lyft', 'doordash', 'instacart', 'pinterest', 'twitter', 'square',
      'coinbase', 'robinhood', 'figma', 'notion', 'linear', 'vercel', 'netlify',
      'supabase', 'planetscale', 'railway', 'render', 'anthropic', 'openai'
    ];
  }
}
